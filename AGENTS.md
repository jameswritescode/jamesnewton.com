...
- **One emitter, two metric declarations.** Events are emitted once (via `Newton.Telemetry`), but each metric built on them is declared in two consumers: `NewtonWeb.Telemetry.metrics/0` (LiveDashboard, dev) and the matching PromEx plugin under `Newton.PromEx.Plugins.*` (Prometheus → Fly Grafana, prod). When adding a metric, add both declarations — one side alone means the metric silently never appears in the other viewer
