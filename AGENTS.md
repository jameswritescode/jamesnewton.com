...
- **Declare custom metrics once, in `Newton.Metrics.definitions/0`.** Events are emitted once (via `Newton.Telemetry`); both viewers consume the same definitions list (`NewtonWeb.Telemetry` → LiveDashboard in dev, the PromEx adapter plugin → Prometheus/Fly Grafana in prod). Use only Prometheus-exportable types — `counter`, `sum`, `last_value`, `distribution` (with explicit `reporter_options: [buckets: [...]]`) — **never `summary`**, which PromEx cannot export
